    ...
    switch (tag >> 3) {
      ...
      case 2:
        if (PROTOBUF_PREDICT_TRUE(static_cast<::PROTOBUF_NAMESPACE_ID::uint8>(tag) == 18)) {
          ...
          do {
            ptr += 1;
            auto str = _internal_add_friendlists();
            ptr = ::PROTOBUF_NAMESPACE_ID::internal::InlineGreedyStringParser(str, ptr, ctx);
            CHK_(ptr);
...
::PROTOBUF_NAMESPACE_ID::uint8* GetFriendListsResponse::_InternalSerialize(
  ...
  // repeated bytes friendlists = 2;
  for (int i = 0, n = this->_internal_friendlists_size(); i < n; i++) {
    const auto& s = this->_internal_friendlists(i);
...
// @@protoc_insertion_point(message_byte_size_start:fixbug.GetFriendListsResponse)
  ...
  // repeated bytes friendlists = 2;
  total_size += 1 *
      ::PROTOBUF_NAMESPACE_ID::internal::FromIntSize(friendlists_.size());
  for (int i = 0, n = friendlists_.size(); i < n; i++) {
